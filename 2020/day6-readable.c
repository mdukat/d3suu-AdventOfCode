...
char* datain[];
char* sanitydata[];
#define DATASIZE 2240
#define SANITYSIZE 16
#define SANITY

// int part_one
// return n_allanswers
int part_one(int datasize, char** datainput){ // for explanation first read part 2
	int n_allanswers = 0;
	for(int i = 0; i<datasize; i++){
		char answers['z'+1];
		int n_answers = 0;
		memset(answers, 0x00, 'z'+1);
		while(datainput[i] != ""){
			for(int j = 0; datainput[i][j]!=0x00; j++){
				answers[datainput[i][j]] = 1; // set answer to 1, so we can count it
			}
#ifdef DEBUG
			printf("DEBUG: %d\n", i);
#endif
		...
		for(int j = 0; j<('z'+1); j++){
			if(answers[j] == 1){ // if answer is 1
#ifdef OUTPUT
				printf("%d", answers[j]);
#endif
				n_answers++; // increment answers
				n_allanswers++;
			}
		}
#ifdef OUTPUT
...
int main(){
	...
	// sanity;
	n_allanswers = part_one(SANITYSIZE, sanitydata);
	printf("sanity all: %d\n", n_allanswers);
	if(n_allanswers != 11){
		printf("Sanity ERROR, no match. %d should be 11\n", n_allanswers);
		return -1;
	}else{
		printf("Sanity OK\n");
	}
	...
	// part 1
	n_allanswers = 0;
	n_allanswers = part_one(DATASIZE, datain);
	printf("part 1 all: %d\n", n_allanswers);
	// aoc gave me partial data and changed it 10 minutes in, very nice

	// part 2
	n_allanswers = 0;
	for(int i = 0; i<DATASIZE; i++){ // for every line in datain[]
		char answers['z'+1]; // table for each answer, dont care about optimalization
		int n_answers = 0; // answers per group
		int i_buffer = 0; // number of people in group, for "everyone's answer" calculation
		memset(answers, 0x00, 'z'+1);

		while(datain[i] != ""){ // aka until line is not "", do
			for(int j = 0; datain[i][j]!=0x00; j++){ // do until character in line is not 0x00 (end of string)
				answers[datain[i][j]] += 1; // +1 to every answer in line
			}
			i_buffer++;
#ifdef DEBUG
			printf("DEBUG: %d, i_buffer: %d\n", i, i_buffer);
#endif
			i++; // manipulate main for() loop, to read whole group instead of line
		}

		for(int j = 0; j<('z'+1); j++){ // for each answer
			if(answers[j] == i_buffer){ // if answered by every person (line), then
#ifdef OUTPUT
				printf("%d", answers[j]);
#endif
				n_answers++; // increment answers
