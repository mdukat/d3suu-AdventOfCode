...
char* datain[] = {"iyr:2010 ecl:gry hgt:181cm",
...
#define DATASIZE 1160
#define BUFFERSIZE 512

#define LINEOUT // full lines with data
//#define SANITY // Sanity check in loop

int main(){
	char* strbuffer = malloc(512);
	memset(strbuffer, 0x00, 512);
	int valid_passports = 0;
	int valid_passports2 = 0;
	
	// Sanity test
	printf("SANITY 1 %ld - %ld\n", strlen(datain[0]), strlen(strbuffer));
	...
	for(int i = 0; i<DATASIZE; i++){
		...
		//while(strcmp(datain[i], datain[1159]) != 0){
		while(datain[i] != ""){
			strcat(strbuffer, " "); // puts space before whole line, but dont care
			strcat(strbuffer, datain[i]);
			i++;
		}
		strcat(strbuffer, " "); // put space on end, for second half
		/*
		byr (Birth Year)
		iyr (Issue Year)
		eyr (Expiration Year)
		...
		*/
		...
		// Search for tags
		int points = 7;
		if(strstr(strbuffer, "byr") == NULL) // Switch..case would be better or
			points--;                    // something, but keep it stupid simple
		if(strstr(strbuffer, "iyr") == NULL)
			points--;
		if(strstr(strbuffer, "eyr") == NULL)
			points--;
		if(strstr(strbuffer, "hgt") == NULL)
			points--;
		if(strstr(strbuffer, "hcl") == NULL)
			points--;
		if(strstr(strbuffer, "ecl") == NULL)
			points--;
		if(strstr(strbuffer, "pid") == NULL)
			points--; // haha points go ----------
		
		if(points == 7)
			valid_passports++; // END OF FIRST HALF
		#ifdef LINEOUT
		printf("%s\n", strbuffer);
		#endif

		// Second half - Check for specific criteria
		if(points == 7){
			// Split line to each tag, then check value
			// Mindmap - strstr -> end-beginning -> sscanf -> test
			
			// _b - beginning
			// _e - ending
			char* byr_b = strstr(strbuffer, "byr");
			char* byr_e = strstr(byr_b, " ");
			char* iyr_b = strstr(strbuffer, "iyr");
			char* iyr_e = strstr(iyr_b, " ");
			char* eyr_b = strstr(strbuffer, "eyr");
			char* eyr_e = strstr(eyr_b, " ");
			char* hgt_b = strstr(strbuffer, "hgt");
			char* hgt_e = strstr(hgt_b, " ");
			char* hcl_b = strstr(strbuffer, "hcl");
			char* hcl_e = strstr(hcl_b, " ");
			char* ecl_b = strstr(strbuffer, "ecl");
			char* ecl_e = strstr(ecl_b, " ");
			char* pid_b = strstr(strbuffer, "pid");
			char* pid_e = strstr(pid_b, " ");

			#ifdef SANITY
			// sanity test
			char sanity_byr[20];
			memset(sanity_byr, 0x00, 20);
			strncat(sanity_byr, byr_b, byr_e-byr_b);
			printf("SANITY BYR %s\n", sanity_byr);
			#endif
			...
			char tagbuffer[20];
			// byr
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, byr_b, byr_e-byr_b);
			int year;
			sscanf(tagbuffer, "byr:%d", &year);
			if(year < 1920 || year > 2002)
				continue;

			// iyr
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, iyr_b, iyr_e-iyr_b);
			sscanf(tagbuffer, "iyr:%d", &year);
			if(year < 2010 || year > 2020)
				continue;

			// eyr
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, eyr_b, eyr_e-eyr_b);
			sscanf(tagbuffer, "eyr:%d", &year);
			...
			memset(tagbuffer, 0x00, 20);
			int height;
			strncat(tagbuffer, hgt_b, hgt_e-hgt_b);
			/*
			if(sscanf(tagbuffer, "hgt:%dcm", &height) == 1){
				// scan for cm
				printf("centimeters\n");
				...
				// scan for in
				printf("inches\n");
				sscanf(tagbuffer, "hgt:%din", &height);
				if(height < 59 || height > 76)
					continue;
			}
			*/
			if(strstr(tagbuffer, "cm") == NULL){
				// scan for in
				printf("inches\n");
				sscanf(tagbuffer, "hgt:%din", &height);
				if(height < 59 || height > 76)
				...
				// scan for cm
				printf("centimeters\n");
				sscanf(tagbuffer, "hgt:%dcm", &height);
				if(height < 150 || height > 193)
					continue;
			}
			...
			// hcl
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, hcl_b, hcl_e-hcl_b);
			// Check for #
			if(tagbuffer[4] != '#')
				continue;
			// Check for length
			if(strlen(tagbuffer) != 11)
				continue;
			// Check for values
			char* hexvalues = "0123456789abcdef";
			int continue_handle = 0;
			for(int vali = 5; vali < strlen(tagbuffer); vali++){
				int hexwrong = strlen(hexvalues);
				for(int hexi = 0; hexi < strlen(hexvalues); hexi++){
					if(tagbuffer[vali] != hexvalues[hexi])
						hexwrong--;
				}
				if(hexwrong != 1){
					printf("HEX WRONG\n");
			...
			if(continue_handle == 1)
				continue;

			// ecl
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, ecl_b, ecl_e-ecl_b);
			if(strstr(tagbuffer, "amb") != NULL){
				printf("amb\n");
			}else if(strstr(tagbuffer, "blu") != NULL){
				printf("blu\n");
			}else if(strstr(tagbuffer, "brn") != NULL){
				printf("brn\n");
			}else if(strstr(tagbuffer, "gry") != NULL){
				printf("gry\n");
			}else if(strstr(tagbuffer, "grn") != NULL){
				printf("grn\n");
			}else if(strstr(tagbuffer, "hzl") != NULL){
				printf("hzl\n");
			}else if(strstr(tagbuffer, "oth") != NULL){
				printf("oth\n");
			}else{
				printf("WRONG EYE COLOR\n");
				continue;
			}
			
			// pid
			memset(tagbuffer, 0x00, 20);
			strncat(tagbuffer, pid_b, pid_e-pid_b);
			// Check for length
			if(strlen(tagbuffer) != 13)
				continue;
			// Check for values
			char* numvalues = "0123456789";
			continue_handle = 0;
			for(int vali = 5; vali < strlen(tagbuffer); vali++){
				int numwrong = strlen(numvalues);
				for(int numi = 0; numi < strlen(numvalues); numi++){
					if(tagbuffer[vali] != numvalues[numi])
						numwrong--;
				}
				if(numwrong != 1){
					printf("PID WRONG\n");
	...
	free(strbuffer);
	printf("First half - Valid passports: %d\n", valid_passports);
	printf("Second half - Valid passports: %d\n", valid_passports2);
	return 0;
}
