...
char* datain[] = {"1-13 r: gqdrspndrpsrjfjx",
...
int main(){
	...
	*/
	...
	char letter;
	char password[50];
	//sscanf(datain[0], "%d-%d %c: %s", &min, &max, &letter, password);
	//printf("TEST PARSE %d %d %c %s SIZE %ld\n", min, max, letter, password, strlen(password));

	// Real parse - First Half
	int N, okpass;
	size_t passlen;
	okpass = 0;
	for(int i = 0; i<DATASIZE; i++){
		N = 0;
		sscanf(datain[i], "%d-%d %c: %s", &min, &max, &letter, password);
	...
	printf("First Half - Good passwords: %d\n", okpass);
	...
	for(int i = 0; i<DATASIZE; i++){
		ok = 0;
		sscanf(datain[i], "%d-%d %c: %s", &first, &second, &letter, password);
		passlen = strlen(password);
		if(first > passlen || second > passlen) // just to be sure
			continue;
		...
		if(ok == 1){ // Only one letter can be ok
			okpass++;
			printf("%s OK\n", password);
		}
	}
	printf("Second half - Good passwords: %d\n", okpass);
