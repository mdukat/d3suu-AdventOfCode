...
int main(){
	...
	// sanity;
#ifdef SANITY
	for(int i = 0; i<SANITYSIZE; i++){
		char answers['z'+1];
		int n_answers = 0;
		memset(answers, 0x00, 'z'+1);
		while(sanitydata[i] != ""){
			for(int j = 0; sanitydata[i][j]!=0x00; j++){
				answers[sanitydata[i][j]] = 1;
			}
#ifdef DEBUG
			printf("DEBUG: %d\n", i);
#endif
	...
	for(int i = 0; i<DATASIZE; i++){
		...
		memset(answers, 0x00, 'z'+1);
		while(datain[i] != ""){
			for(int j = 0; datain[i][j]!=0x00; j++){
				answers[datain[i][j]] = 1;
			}
#ifdef DEBUG
			printf("DEBUG: %d\n", i);
#endif
